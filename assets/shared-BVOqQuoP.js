import{K as p,bg as I}from"./BridgePanel-BexIBQgO.js";import"./vendor-markdown-kWDvDbaY.js";import"./vendor-react-C0wFb-u9.js";import"./ErrorNotificationContext-ombTHrqk.js";import"./index-BoZ-GOx6.js";import"./StripeService-hcnm9rsc.js";import"./TauriBootstrapGate-Cs0Z_k12.js";import"./webviewSuppressStore-D-BvO955.js";import"./logoSymbol-m3hY5gIG.js";import"./AuthConfig-CtRI5sq4.js";import"./vendor-codemirror-C_N5vxNQ.js";function v(){return`#!/usr/bin/env node
const fs = require('fs');
const os = require('os');
...
// The CLI config home this agent process runs under (AI account profiles).
// Conversation/thread ids are scoped to it, so a resume must execute under
// the SAME dir. Empty = the CLI's default home.
let configDir = '';
if (agent === 'codex') {
  configDir = typeof process.env.CODEX_HOME === 'string' ? process.env.CODEX_HOME : '';
} else if (agent === 'claude') {
  configDir = typeof process.env.CLAUDE_CONFIG_DIR === 'string' ? process.env.CLAUDE_CONFIG_DIR : '';
}
...
const dropPayload = {
  ...
  configDir: configDir ? truncate(configDir, 512) : '',
};
...
process.exit(0);
`}function y(){return window.tauriAPI?.system?.platform??"darwin"}function f(e,t=y()){return t==="win32"?`"${e.replace(/"/g,'\\"')}"`:`'${e.replace(/'/g,"'\\''")}'`}async function x(){const e=await window.tauriAPI.system.getHomeDir(),t=I(e,".bridgespace","bin","bs-agent-notify.cjs");return p(t)}function N(e){const{helperPath:t,agent:s,event:n,fallbackTitle:h,fallbackBody:m}=e,o=e.platform??y(),g=m??"Agent update",w=`node ${f(t,o)} --agent ${s} --event ${n} "$@"`,i=g.replace(/"/g,'\\"'),d=h.replace(/"/g,'\\"');let r;if(o==="darwin")r=`osascript -e 'display notification "${i}" with title "${d}"' 2>/dev/null || true`;else if(o==="win32"){const c=t.replace(/'/g,"''"),l=s.replace(/'/g,"''"),u=n.replace(/'/g,"''");return`powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "try { $payload = [Console]::In.ReadToEnd(); if ($payload.Length -gt 0) { $payload | & node '${c}' --agent '${l}' --event '${u}' 2>$null | Out-Null } else { & node '${c}' --agent '${l}' --event '${u}' 2>$null | Out-Null } } catch { } finally { exit 0 }"`}else r=`(command -v notify-send >/dev/null 2>&1 && notify-send "${d}" "${i}") || true`;const b=`${w} 2>/dev/null || ${r}; true`;return`sh -c ${f(b,"darwin")} -- "$@"`}async function a(e){const t=p(e),s=t.lastIndexOf("/");if(s<=0)return;const n=t.slice(0,s);try{await window.tauriAPI.fs.createDirectory(n)}catch{}}async function R(e,t){try{return await window.tauriAPI.fs.readFile(e)}catch{return t}}async function M(e){try{const t=await window.tauriAPI.fs.readFile(e),s=JSON.parse(t);if(s&&typeof s=="object"&&!Array.isArray(s))return s}catch{}return{}}async function $(e,t){await a(e),await window.tauriAPI.fs.writeFile(e,`${JSON.stringify(t,null,2)}
`)}async function L(e,t){await a(e),await window.tauriAPI.fs.writeFile(e,t)}async function H(e,t){try{return(await window.tauriAPI.fs.readFile(e)).includes(t)}catch{return!1}}async function B(){const e=await x();await a(e),await window.tauriAPI.fs.writeFile(e,v());try{await window.tauriAPI.fs.setExecutable?.(e)}catch{}return e}const J="bs-agent-notify.cjs";export{J as BS_HOOK_MARKER,N as buildHookCommand,B as ensureAgentNotifyHelperInstalled,a as ensureParentDir,H as fileContains,x as getAgentNotifyHelperPath,f as quotePathForShell,R as readFileOr,M as readJsonOrEmpty,$ as writeJsonFile,L as writeTextFile};
